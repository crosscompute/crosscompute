...
function import_upload(e, d, import_url, update_argument) {
  var $o = $(e.target), $feedback = $o.next('.feedback'),
    argument_name = $o.data('name'), $argument = $('#' + argument_name);

  $feedback.text('Loading... Large uploads can take some time to process');

  $.post(import_url, {
    upload_id: d.upload_id,
    argument_name: argument_name
  }).fail(function(x) {
    $feedback.addClass('error').html(x.responseJSON[argument_name]);
  }).success(function(x) {
    $feedback.removeClass('error').text('');
    $argument.html(x);
    $('[name=' + argument_name + ']').val(d.upload_id);
    update_argument && update_argument($feedback, $argument);
  });
}
